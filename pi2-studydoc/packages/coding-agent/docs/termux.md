...
2. 从 GitHub 或 F-Droid 安装 [Termux:API](https://github.com/termux/termux-api#installation)，以使用剪贴板及其他设备集成功能
...
在 Termux 中运行时，剪贴板操作使用 `termux-clipboard-set` 和 `termux-clipboard-get`。这些功能需要安装 Termux:API 应用才能工作。
...
- **存储访问**：要访问 `/storage/emulated/0`（Downloads 等）中的文件，请运行一次 `termux-setup-storage` 来授予权限
